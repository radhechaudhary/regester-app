...
useEffect(()=>{         // useEffect which triggers when login data is changed connects with the databse and check whether data is valid or not
      axios.post("http://localhost:4000/login",{username:loginData.username, password:loginData.password})
      .then((response)=>{
...
useEffect(()=>{          // useState which triggers when signUpData is updated everytime
  axios.post("http://localhost:4000/signup",{name:signUpData.name, mobile:signUpData.mobile, username:signUpData.username, password:signUpData.password})
  .then((response)=>{
      if(signUpData.name){
